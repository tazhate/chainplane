...
const (
	...
	// FinalizerName is the finalizer added to ChainInstance resources to
	// ensure proper cleanup of owned sub-resources before deletion.
	FinalizerName = "chains.chainplane.io/finalizer"
)
...
// ---------------------------------------------------------------------------
// ChainInstanceSpec — desired state
// ---------------------------------------------------------------------------

// ChainInstanceSpec defines the desired state of a ChainInstance resource.
...
// ---------------------------------------------------------------------------
// ChainInstanceStatus — observed state
// ---------------------------------------------------------------------------

// ChainInstanceStatus defines the observed state of ChainInstance as
// reported by the controller's reconciliation loop and health checks.
type ChainInstanceStatus struct {
...
// ChainInstance manages the full lifecycle of a blockchain node: StatefulSet,
...
type ChainInstance struct {
	...
	Spec   ChainInstanceSpec   `json:"spec,omitempty"`
	Status ChainInstanceStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// ChainInstanceList contains a list of ChainInstance resources.
type ChainInstanceList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []ChainInstance `json:"items"`
}

func init() {
	SchemeBuilder.Register(&ChainInstance{}, &ChainInstanceList{})
}
