...
var webhookLog = logf.Log.WithName("blockchainnode-webhook")
...
func (r *BlockchainNode) SetupWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr).
		For(r).
		WithValidator(&BlockchainNodeValidator{}).
		Complete()
}
...
// ValidateCreate validates a new BlockchainNode resource.
func (v *BlockchainNodeValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	node, ok := obj.(*BlockchainNode)
	if !ok {
		return nil, fmt.Errorf("expected a BlockchainNode object but got %T", obj)
	}
	webhookLog.Info("validate create", "name", node.Name)
	warnings, err := validateBlockchainNodeSpec(node)
	return warnings, err
}

// ValidateUpdate validates an update to an existing BlockchainNode resource.
// Chain and network fields are immutable.
func (v *BlockchainNodeValidator) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	oldNode, ok := oldObj.(*BlockchainNode)
	if !ok {
		return nil, fmt.Errorf("expected a BlockchainNode object but got %T", oldObj)
	}
	newNode, ok := newObj.(*BlockchainNode)
	if !ok {
		return nil, fmt.Errorf("expected a BlockchainNode object but got %T", newObj)
	}
	...
	warnings, specErr := validateBlockchainNodeSpec(newNode)
	if specErr != nil {
...
// ValidateDelete validates deletion of a BlockchainNode (always permitted).
func (v *BlockchainNodeValidator) ValidateDelete(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	node, ok := obj.(*BlockchainNode)
	if !ok {
		return nil, fmt.Errorf("expected a BlockchainNode object but got %T", obj)
	}
...
// validateBlockchainNodeSpec performs field-level validation on the BlockchainNode spec.
...
func validateBlockchainNodeSpec(node *BlockchainNode) (admission.Warnings, error) {
	var (
