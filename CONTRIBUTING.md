...
kubectl apply -f config/samples/nodes_v1alpha1_chaininstance_bitcoin.yaml
...
After modifying CRD types in `api/v1alpha2/chaininstance_types.go`:
...
config/samples/           Example ChainInstance manifests (one per chain)
charts/                   Helm chart
...
In `api/v1alpha2/chaininstance_types.go`, add a new `Chain` constant and include it in the `+kubebuilder:validation:Enum` comment on the `Chain` type:
...
Create `config/samples/nodes_v1alpha1_chaininstance_mychain.yaml` and add it to `config/samples/kustomization.yaml`.
