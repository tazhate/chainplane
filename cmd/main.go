...
const (
	...
	defaultLeaderElectID = "f8c0f89a.chains.chainplane.io"
)
...
func init() {
	...
	utilruntime.Must(chainsv1alpha2.AddToScheme(scheme))
...
// setupBlockchainController creates and registers the ChainInstance reconciler.
func setupBlockchainController(mgr ctrl.Manager) error {
	...
	reconciler := &controller.ChainInstanceReconciler{
	...
	if err := reconciler.SetupWithManager(mgr); err != nil {
		return fmt.Errorf("creating ChainInstance controller: %w", err)
	}
	return nil
}
...
func main() {
	...
	if err := setupBlockchainController(mgr); err != nil {
		setupLog.Error(err, "controller setup failed", "controller", "ChainInstance")
		os.Exit(1)
	}
