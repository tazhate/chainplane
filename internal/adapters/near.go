...
func init() {
	Register(nodesv1alpha1.ChainNear, &nearAdapter{
		baseAdapter: baseAdapter{livenessPort: 3030},
	})
}
...
func (a *nearAdapter) HealthCheck(ctx context.Context, rpcURL string) (SyncStatus, error) {
	...
	if err != nil {
		return SyncStatus{}, fmt.Errorf("near status request: %w", err)
	}
	...
	if err != nil {
		return SyncStatus{}, fmt.Errorf("near status read: %w", err)
	}

	var status nearStatusResponse
	if err := json.Unmarshal(body, &status); err != nil {
		return SyncStatus{}, fmt.Errorf("near status parse: %w", err)
	}
...
func (a *nearAdapter) StartupProbe(_ nodesv1alpha1.BlockchainNodeSpec) *corev1.Probe {
	return httpProbe("/status", 3030, 60, 30, 10, 480)
}

func (a *nearAdapter) LivenessProbe(_ nodesv1alpha1.BlockchainNodeSpec) *corev1.Probe {
	return httpProbe("/status", 3030, 60, 30, 10, 5)
}

func (a *nearAdapter) ContainerCommand(_ nodesv1alpha1.BlockchainNodeSpec) []string {
	...
	// stuck in the "header" phase with "Unexpected state response (wrong sender)" errors.
