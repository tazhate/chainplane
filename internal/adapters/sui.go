...
func init() {
	Register(chainsv1alpha2.ChainSui, &suiAdapter{
		protocolAdapter: protocolAdapter{livenessPort: 9000},
	})
}
...
func (a *suiAdapter) DefaultImage(client string) string {
	return DefaultImageFor(chainsv1alpha2.ChainSui, client)
}

func (a *suiAdapter) ConfigTemplate(_ chainsv1alpha2.ChainInstanceSpec) (string, string, error) {
...
func (a *suiAdapter) HealthCheck(ctx context.Context, rpcURL string) (SyncStatus, error) {
	...
	if err != nil {
		return SyncStatus{}, fmt.Errorf("decode SUI RPC URL: %w", err)
	}
	metricsURL := "http://" + u.Hostname() + ":9184/metrics"

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, metricsURL, nil)
	if err != nil {
		return SyncStatus{}, fmt.Errorf("scrape SUI metrics endpoint: %w", err)
	}
	...
	if err != nil {
		return SyncStatus{}, fmt.Errorf("read SUI metrics response: %w", err)
	}
...
func (a *suiAdapter) StartupProbe(_ chainsv1alpha2.ChainInstanceSpec) *corev1.Probe {
	return tcpProbe(9000, 30, 30, 10, 120)
}

func (a *suiAdapter) ContainerCommand(_ chainsv1alpha2.ChainInstanceSpec) []string {
	return []string{
		"sh", "-c",
...
func (a *suiAdapter) InitContainers(_ chainsv1alpha2.ChainInstanceSpec) []corev1.Container {
...
if [ -d /data/db/live ]; then
  ...
  for sub in checkpoints store/perpetual; do
    ...
    if [ -d "$dir" ] && [ ! -f "$dir/CURRENT" ]; then
      echo "sui-db: data dir present but CURRENT marker missing - assuming corrupted DB"
      DB_OK=false
    fi
  done
...
func (a *suiAdapter) ContainerPorts(_ chainsv1alpha2.ChainInstanceSpec) []corev1.ContainerPort {
