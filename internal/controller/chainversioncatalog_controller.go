...
func (r *ChainVersionCatalogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	catalog := &nodesv1alpha1.ChainVersionCatalog{}
	...
	recentTags := make([]nodesv1alpha1.TagInfo, 0, limit)
	for _, e := range sorted[:limit] {
		ti := nodesv1alpha1.TagInfo{Tag: e.Tag}
		if e.PublishedAt != "" {
...
func (r *ChainVersionCatalogReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&nodesv1alpha1.ChainVersionCatalog{}).
		Complete(r)
}
...
func setReachableCondition(catalog *nodesv1alpha1.ChainVersionCatalog, status metav1.ConditionStatus, reason, message string) {
	meta.SetStatusCondition(&catalog.Status.Conditions, metav1.Condition{
		Type:               nodesv1alpha1.ConditionRegistryReachable,
		Status:             status,
		Reason:             reason,
		Message:            message,
