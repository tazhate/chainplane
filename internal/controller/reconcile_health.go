...
const (
	...
	annotationDegradedSince = "chains.chainplane.io/degraded-since"

	// annotationSyncStallSince / annotationLastSyncBlock track height
	// freeze detection across reconcile cycles.
	annotationSyncStallSince = "chains.chainplane.io/sync-stall-since"
	annotationLastSyncBlock  = "chains.chainplane.io/last-sync-block"

	// Progress-snapshot annotations for ETA calculation.
	annotationProgressSnapshotPct = "chains.chainplane.io/progress-snapshot-pct"
	annotationProgressSnapshotAt  = "chains.chainplane.io/progress-snapshot-at"

	// Block-rate snapshot annotations (fallback ETA method).
	annotationBlockSnapshotHeight = "chains.chainplane.io/block-snapshot-height"
	annotationBlockSnapshotAt     = "chains.chainplane.io/block-snapshot-at"
...
// status patch.
func (r *ChainInstanceReconciler) refreshStatus(
	ctx context.Context,
	...
	if node.Status.Phase == chainsv1alpha2.NodePhaseDegraded {
...
func (r *ChainInstanceReconciler) applyDegradedFromError(
	node *chainsv1alpha2.ChainInstance,
	healthErr error,
	now metav1.Time,
) {
	node.Status.Phase = chainsv1alpha2.NodePhaseDegraded
	metrics.RecordPhase(node, chainsv1alpha2.NodePhaseDegraded)
	setCondition(node, chainsv1alpha2.ConditionDegraded, metav1.ConditionTrue, "HealthCheckFailed", healthErr.Error(), now)
	setCondition(node, chainsv1alpha2.ConditionReady, metav1.ConditionFalse, "HealthCheckFailed", "RPC health check failed", now)
}

// applyHealthyOrSyncing updates status based on the adapter's sync report.
func (r *ChainInstanceReconciler) applyHealthyOrSyncing(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
...
func (r *ChainInstanceReconciler) applySyncingStatus(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	status adapters.SyncStatus,
	now metav1.Time,
	...
	if node.Spec.Chain == chainsv1alpha2.ChainAvalanche && status.Progress == 0 {
	...
	node.Status.Phase = chainsv1alpha2.NodePhaseSyncing
	if !preserveProgress {
		pct := status.Progress
		if pct > 99.9 {
			pct = 99.9
	...
	setCondition(node, chainsv1alpha2.ConditionSyncing, metav1.ConditionTrue, "Syncing", fmt.Sprintf("%.1f%% complete", status.Progress), now)
	setCondition(node, chainsv1alpha2.ConditionReady, metav1.ConditionFalse, "Syncing", "Node is syncing", now)
	setCondition(node, chainsv1alpha2.ConditionDegraded, metav1.ConditionFalse, "Syncing", "Node is syncing", now)
}

// applyFullySyncedStatus handles the IsSyncing=false branch.
func (r *ChainInstanceReconciler) applyFullySyncedStatus(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	status adapters.SyncStatus,
	now metav1.Time,
	...
	if stalled, msg := r.detectHeightStall(ctx, node, status.CurrentBlock, logger); stalled {
		node.Status.Phase = chainsv1alpha2.NodePhaseDegraded
		metrics.RecordPhase(node, chainsv1alpha2.NodePhaseDegraded)
		setCondition(node, chainsv1alpha2.ConditionDegraded, metav1.ConditionTrue, "HeightStall", msg, now)
		setCondition(node, chainsv1alpha2.ConditionReady, metav1.ConditionFalse, "HeightStall", msg, now)
	...
	node.Status.Phase = chainsv1alpha2.NodePhaseHealthy
	node.Status.SyncProgress = "100%"
	node.Status.SyncETA = ""
	setCondition(node, chainsv1alpha2.ConditionSyncing, metav1.ConditionFalse, "Synced", "Node is fully synced", now)
	...
	if lagging, lag := r.checkBlockLag(ctx, node, status.CurrentBlock); lagging {
		...
		setCondition(node, chainsv1alpha2.ConditionDegraded, metav1.ConditionTrue, "BlockLag", msg, now)
		setCondition(node, chainsv1alpha2.ConditionReady, metav1.ConditionFalse, "BlockLag", msg, now)
	} else {
		setCondition(node, chainsv1alpha2.ConditionDegraded, metav1.ConditionFalse, "Healthy", "Node is healthy", now)
		setCondition(node, chainsv1alpha2.ConditionReady, metav1.ConditionTrue, "Healthy", "Node is healthy and synced", now)
	}
}

// markStalled sets the node to Degraded with stall information.
func (r *ChainInstanceReconciler) markStalled(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	progress float64,
	stallMsg string,
	...
	node.Status.Phase = chainsv1alpha2.NodePhaseDegraded
	metrics.RecordPhase(node, chainsv1alpha2.NodePhaseDegraded)
	setCondition(node, chainsv1alpha2.ConditionDegraded, metav1.ConditionTrue, "SyncStall", stallMsg, now)
	setCondition(node, chainsv1alpha2.ConditionReady, metav1.ConditionFalse, "SyncStall", stallMsg, now)
	setCondition(node, chainsv1alpha2.ConditionSyncing, metav1.ConditionTrue, "Syncing", fmt.Sprintf("%.1f%% complete (stalled)", progress), now)
...
func (r *ChainInstanceReconciler) detectHeightStall(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	currentBlock int64,
	logger interface {
...
func (r *ChainInstanceReconciler) checkBlockLag(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	currentBlock int64,
) (bool, int64) {
...
func (r *ChainInstanceReconciler) restartIfDegradedTooLong(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
) error {
...
func (r *ChainInstanceReconciler) clearDegradedTracking(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	logger interface {
		Error(error, string, ...interface{})
...
func (r *ChainInstanceReconciler) avaxBootstrapProgress(ctx context.Context, node *chainsv1alpha2.ChainInstance) float64 {
...
func (r *ChainInstanceReconciler) estimateSyncETA(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	progress float64,
	currentBlock int64,
...
func (r *ChainInstanceReconciler) progressBasedETA(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	progress float64,
	now time.Time,
...
func (r *ChainInstanceReconciler) blockRateETA(
	ctx context.Context,
	node *chainsv1alpha2.ChainInstance,
	currentBlock int64,
	now time.Time,
...
func ensureAnnotations(node *chainsv1alpha2.ChainInstance) {
