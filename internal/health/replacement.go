...
// Annotation keys used to track replacement state on the ChainInstance resource.
const (
	AnnotationReplacementPhase     = "chains.chainplane.io/replacement-phase"
	AnnotationReplacementPod       = "chains.chainplane.io/replacement-pod"
	AnnotationReplacementOldPod    = "chains.chainplane.io/replacement-old-pod"
	AnnotationReplacementStartedAt = "chains.chainplane.io/replacement-started-at"
)

// Label keys applied to pods managed by the replacement workflow.
const (
	LabelInstance       = "chains.chainplane.io/instance"
	LabelReplacementFor = "chains.chainplane.io/replacement-for"
	LabelReady          = "chains.chainplane.io/ready"
)
...
// StartReplacement initiates the replacement workflow for a ChainInstance.
// It sets the phase to Draining, cordons the old pod, drains traffic,
// creates a replacement pod, and advances to Verifying.
func (r *ReplacementManager) StartReplacement(ctx context.Context, node *chainsv1alpha2.ChainInstance) error {
...
func (r *ReplacementManager) MonitorReplacement(ctx context.Context, node *chainsv1alpha2.ChainInstance) (bool, error) {
	log := slog.With("node", node.Name, "namespace", node.Namespace)
...
func (r *ReplacementManager) CompleteReplacement(ctx context.Context, node *chainsv1alpha2.ChainInstance) error {
	log := slog.With("node", node.Name, "namespace", node.Namespace)
...
func (r *ReplacementManager) RollbackReplacement(ctx context.Context, node *chainsv1alpha2.ChainInstance) error {
	log := slog.With("node", node.Name, "namespace", node.Namespace)
...
func IsReplacementInProgress(node *chainsv1alpha2.ChainInstance) bool {
	phase := nodeAnnotation(node, AnnotationReplacementPhase)
...
func GetReplacementPhase(node *chainsv1alpha2.ChainInstance) ReplacementPhase {
...
func (r *ReplacementManager) spawnReplacementPod(
	...
	labels["chains.chainplane.io/chain"] = string(node.Spec.Chain)
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				"chains.chainplane.io/original-pod": oldName,
				"chains.chainplane.io/created":      time.Now().UTC().Format(time.RFC3339),
			},
		},
...
func (r *ReplacementManager) mergeAnnotations(
	...
	var current chainsv1alpha2.ChainInstance
	if err := r.kube.Get(ctx, types.NamespacedName{
...
// nodeAnnotation safely reads an annotation from a ChainInstance.
func nodeAnnotation(node *chainsv1alpha2.ChainInstance, key string) string {
