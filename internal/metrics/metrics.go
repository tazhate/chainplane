...
func RecordSyncStatus(node *nodesv1alpha1.BlockchainNode, status adapters.SyncStatus) {
...
func RecordAutoRestart(node *nodesv1alpha1.BlockchainNode, reason string, degradedSeconds float64) {
...
func RecordDegradedRecovery(node *nodesv1alpha1.BlockchainNode, degradedSeconds float64) {
	degradedDuration.With(prometheus.Labels{
		"chain":   string(node.Spec.Chain),
		"network": string(node.Spec.Network),
	}).Observe(degradedSeconds)
...
func RecordPhase(node *nodesv1alpha1.BlockchainNode, phase nodesv1alpha1.NodePhase) {
	phases := []nodesv1alpha1.NodePhase{
		nodesv1alpha1.NodePhasePending,
		nodesv1alpha1.NodePhaseSyncing,
		nodesv1alpha1.NodePhaseHealthy,
		nodesv1alpha1.NodePhaseDegraded,
		nodesv1alpha1.NodePhaseFailed,
	}
