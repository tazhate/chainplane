...
func (s *Server) refreshCache(ctx context.Context) {
	list := &v1alpha2.ChainInstanceList{}
...
// nodeToInfo converts a ChainInstance CR to the API type.
func nodeToInfo(n *v1alpha2.ChainInstance) NodeInfo {
	replicas := int32(1)
...
func buildSummary(nodes []NodeInfo) Summary {
	...
	for _, n := range nodes {
		switch v1alpha2.NodePhase(n.Phase) {
		case v1alpha2.NodePhaseHealthy:
			s.Healthy++
		case v1alpha2.NodePhaseSyncing:
			s.Syncing++
		case v1alpha2.NodePhaseDegraded:
			s.Degraded++
		case v1alpha2.NodePhaseFailed:
			s.Failed++
		default:
			s.Pending++
		}
