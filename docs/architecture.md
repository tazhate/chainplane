...
The primary CRD. Each instance describes one blockchain node: the chain type (via `.spec.chain`), the container image or version tracking policy, resource requirements, storage, networking, and health thresholds. The operator owns all child resources created from a ChainInstance.

### ChainVersionCatalog

A cluster-scoped CRD that holds discovered image tags per chain. The operator polls configured container registries on a schedule and writes the latest resolved tag back into this resource. ChainInstanceReconciler reads from it when `VersionPolicy` tracking is enabled on a node.

## Component Diagram

```mermaid
graph TD
    User([User / GitOps]) -->|apply ChainInstance CR| K8sAPI[Kubernetes API]

    K8sAPI --> BNR[ChainInstanceReconciler]
...
1. **Fetch CR** — load the `ChainInstance` object; requeue on not-found after a short delay.
...
The catalog controller polls each registry on a configurable interval (default 1 h), collects all matching tags, applies semver normalization and sorting, and writes the latest resolved tag into the `ChainVersionCatalog` status. The `ChainInstanceReconciler` reads this value during `reconcileUpgrade`.
