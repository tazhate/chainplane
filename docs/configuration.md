...
Complete reference for the `ChainInstance` custom resource spec, status fields, and chain-specific configuration.
...
When `spec.resources` is **not** set on a `ChainInstance`, the validating webhook compares the requested resources against the adapter's defaults and emits a **Warning** (not a rejection) if they fall below the recommended minimums. The node is still created — the warning is informational, surfaced in `kubectl apply` output and in the webhook audit log.
...
- **Retain policy:** Consider `reclaimPolicy: Retain` to prevent accidental data loss. PVCs are not deleted when the ChainInstance resource is removed.
