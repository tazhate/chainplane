...
```bash
kubectl describe chaininstance bitcoin-mainnet
```
...
```bash
kubectl patch chaininstance bitcoin-mainnet --type merge -p '{"spec":{"replicas":0}}'
```

Resume by setting replicas back to 1:

```bash
kubectl patch chaininstance bitcoin-mainnet --type merge -p '{"spec":{"replicas":1}}'
```
...
```bash
kubectl delete chaininstance bitcoin-mainnet
```
...
2. Check peer count in `kubectl get chaininstances` -- zero peers means the node cannot connect to the network.
...
**Check:**
1. `kubectl describe chaininstance <name>` -- look at the Conditions section for details.
...
3. CRDs are installed: `kubectl get crd chaininstances.chains.chainplane.io`
