...
Edit `api/v1alpha2/chaininstance_types.go`:
...
func (a *mychainAdapter) ConfigTemplate(_ chainsv1alpha2.ChainInstanceSpec) (string, string, error) {
...
func (a *mychainAdapter) ContainerPorts(_ chainsv1alpha2.ChainInstanceSpec) []corev1.ContainerPort {
...
Every adapter **must** implement `DefaultResources()`. Return a `ResourceDefaults` struct with the recommended CPU request, memory request, and storage size based on the chain's official node documentation. The validating webhook uses these values to warn operators when a `ChainInstance` is created with resources below the recommended minimums.
...
```go
case chainsv1alpha2.ChainMyChain:
    return "snapshots-mychain"
```

## Step 7: Add Webhook Validation

Edit `api/v1alpha2/chaininstance_webhook.go`, add to `validationRegistry`:

```go
chainsv1alpha2.ChainMyChain: {MinStorage: resource.MustParse("100Gi"), MinMemory: resource.MustParse("4Gi")},
```

## Step 8: Create Sample CR

Create `config/samples/nodes_v1alpha1_chaininstance_mychain.yaml`:
...
- `api/v1alpha2/chaininstance_webhook_test.go` — add validation test case

## Checklist

- [ ] Chain constant in `chaininstance_types.go`
