...
```
                      ChainInstance CR
                       /            \
                      /              \
       ChainInstanceReconciler    NodeHealthReconciler
...
**ChainInstanceReconciler** (30s reconciliation):
...
Every 30 seconds, the main controller calls the adapter's `HealthCheck(ctx, rpcURL)` method for each ChainInstance. The adapter makes chain-specific RPC calls and returns a `SyncStatus`:
...
The controller tracks block height via annotations (`chains.chainplane.io/last-sync-block`, `chains.chainplane.io/sync-stall-since`). If block height stops advancing during sync, the adapter's `StallExempt` flag determines whether this is expected:
...
ETA is displayed in the `syncETA` status field and the `ETA` column in `kubectl get chaininstances`.
...
When any health trigger fires, the `NodeHealthReconciler` initiates a blue/green pod replacement. The workflow progresses through six phases, tracked via annotations on the ChainInstance resource.
...
**1. Draining**
- Old pod is marked `chains.chainplane.io/ready=false` (removes it from Service endpoints)
...
- Replacement pod gets labels: `chains.chainplane.io/instance=replacement`, `chains.chainplane.io/replacement-for=<old-pod>`, `chains.chainplane.io/ready=false`
...
|------------|-------------|
| `chains.chainplane.io/replacement-phase` | Current phase: `Draining`, `Creating`, `Verifying`, `Completing`, `RollingBack` |
| `chains.chainplane.io/replacement-pod` | Name of the replacement pod |
| `chains.chainplane.io/replacement-old-pod` | Name of the original pod being replaced |
| `chains.chainplane.io/replacement-started-at` | RFC3339 timestamp of when replacement started |
...
Traffic management uses a label-based approach. Services must include `chains.chainplane.io/ready=true` in their selector. Toggling this label on pods controls whether the Kubernetes endpoint controller includes them in the Service's endpoint list.

### Drain Operation

1. Set `chains.chainplane.io/ready=false` on the pod
...
- alert: ChainInstanceDegradedLong
...
- alert: ChainInstanceExcessiveRestarts
...
- alert: ChainInstanceSyncStalled
...
- alert: ChainInstanceDegradedEpisodeLong
