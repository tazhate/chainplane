...
The `BlockchainNodeReconciler` compares the running pod's image tag against `latestTag`. If they differ and `spec.autoUpgrade` is enabled, a rolling restart is triggered with the new tag.
