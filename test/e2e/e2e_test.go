...
var _ = Describe("Manager", Ordered, func() {
	...
	Context("ChainInstance CR lifecycle", func() {
		...
		// cleanupCR deletes a ChainInstance CR by name and waits for it to disappear.
		cleanupCR := func(name string) {
			cmd := exec.Command("kubectl", "delete", "chaininstance", name,
				"-n", testNS, "--ignore-not-found=true", "--timeout=60s")
			_, _ = utils.Run(cmd)
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "chaininstance", name,
					"-n", testNS, "-o", "name")
				output, err := utils.Run(cmd)
		...
		// applyCR applies a ChainInstance CR from a YAML string and returns the CR name.
		applyCR := func(yaml string) string {
			...
			_, err = utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred(), "Failed to apply ChainInstance CR")
			return tmpFile
		}
...
`, crName, testNS)
			...
			By("applying the bitcoin ChainInstance CR")
			applyCR(cr)
...
`, crName, testNS)
			...
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "chaininstance", crName,
...
`, crName, testNS)
			...
			By("applying the bitcoin ChainInstance CR with replicas=1")
			applyCR(cr)
			defer cleanupCR(crName)
			...
			cmd := exec.Command("kubectl", "patch", "chaininstance", crName,
...
`, crName, testNS)
			...
			By("deleting the ChainInstance CR")
			cmd := exec.Command("kubectl", "delete", "chaininstance", crName,
				"-n", testNS, "--timeout=60s")
			_, err := utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred(), "Failed to delete ChainInstance CR")
...
`, ethName, testNS)
			...
			cmd := exec.Command("kubectl", "get", "chaininstance", btcName,
				"-n", testNS, "-o", "jsonpath={.spec.chain}")
			btcChain, err := utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred())
			Expect(btcChain).To(Equal("bitcoin"))

			cmd = exec.Command("kubectl", "get", "chaininstance", ethName,
				"-n", testNS, "-o", "jsonpath={.spec.chain}")
