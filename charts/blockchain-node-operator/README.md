...
A Helm chart for the Blockchain Node Operator - a Kubernetes operator that manages the full lifecycle of blockchain nodes including StatefulSets, PVCs, Services, ConfigMaps, and health monitoring.
...
```bash
helm repo add blockchain-node-operator https://tazhate.github.io/blockchain-node-operator
helm install blockchain-node-operator blockchain-node-operator/blockchain-node-operator
```
...
```bash
kubectl apply -f charts/blockchain-node-operator/templates/crds/
```

## Uninstallation

```bash
helm uninstall blockchain-node-operator
```

CRDs are not removed on uninstall. To remove them manually:

```bash
kubectl delete crd blockchainnodes.nodes.k8s-bch.io
```
